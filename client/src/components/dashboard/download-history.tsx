...
export function DownloadHistoryComponent() {
    const token = (typeof window !== 'undefined' && localStorage.getItem("finsync_token")) || "";
    const sessionId = (typeof window !== 'undefined' && localStorage.getItem("finsync_session_id")) || "";

    const { data: downloads, isLoading } = useQuery<DownloadHistory[]>({
        queryKey: ["download-history", token, sessionId],
        queryFn: async (): Promise<DownloadHistory[]> => {
            try {
                ...
                let url = '/api/download-history'; // Use relative path
                if (token) {
                    headers['Authorization'] = `Bearer ${token}`;
                } else if (sessionId) {
                    url += `?session_id=${encodeURIComponent(sessionId)}`;
                } else {
                    console.warn("No authentication found - neither session_id nor token");
                    return [];
                }

                const res = await fetch(url, { headers, credentials: 'include' });
                if (!res.ok) {
                    console.error(`Failed to fetch download history: ${res.status} ${res.statusText}`);
                ...
                return (rows || []).map((r) => ({
                    ...
                    fileSize: r.fileSize || null, // Ensure it's not undefined
                    downloadedAt: r.downloadedAt ? new Date(r.downloadedAt) : null,
                }));
            } catch (err) {
        ...
        >
            ...
            {downloads?.length === 0 ? (
                ...
                <motion.div variants={containerVariants} className="space-y-2 px-3">
                    {(downloads || []).slice(0, 5).map((download: DownloadHistory) => (
                        <motion.div
                            key={download.id}
                            variants={itemVariants}
                            whileHover={{
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-2 mt-1">
                                        <div className="flex items-center gap-1 text-xs text-gray-500 dark:text-gray-400">
                                            ...
                                            <span>{download.invoicesCount || '0'} invoices</span>
                                        </div>
